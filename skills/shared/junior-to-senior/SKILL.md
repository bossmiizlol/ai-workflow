...
Both are altitude failures. The senior's job is to drag the plan to the right altitude _and_ upgrade its substance past the model's training cutoff.
...
The senior is not a tone of voice. It is a reviewer profile built from research done _now_. Skipping this phase and going straight to critique produces generic review slop.
...
- Declared versions in package manifests (do not read lockfiles prohibited by AGENTS.md) — a plan recommending an API that the pinned version doesn't have is a blocker.
...
When the harness supports subagents, run the senior review in a context-isolated subagent that receives the frozen artifact and the research findings but _not_ the reasoning that produced the junior plan. Self-review in the same context anchors on its own justifications; isolation is what makes the review adversarial rather than confirmatory.
...
6. **Open questions for a human** — product decisions the senior is _not_ allowed to invent. Scoping is the senior's job; product direction is not.
