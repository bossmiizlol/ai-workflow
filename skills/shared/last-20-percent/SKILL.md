...
1. **Tasks come from the noun.** Decomposing "wiki" yields ingestion, storage, search, generation. The experience of _using_ the wiki never appears in that decomposition — so it never becomes a task, so it never gets done.
...
The scene is the destination for everything that follows. If you cannot write it, that gap _is_ the missing 20% — stop and close it first. Propose two or three candidate scenes with a recommendation; never hand the user a blank page to fill in.
